    ...
    def _import_database(self, source_dir: Path) -> None:
        ...
        for folder_data in data.get("folders", []):
            ...
            try:
                ...
                import_errors.append(f"导入文件夹 {folder.name} 失败: {e}")
        ...
        for tag_data in data.get("tags", []):
            ...
            try:
                ...
                import_errors.append(f"导入标签 {tag.name} 失败: {e}")
        ...
        for doc_data in data.get("documents", []):
            ...
            try:
                ...
                import_errors.append(f"导入文档 {doc.title} 失败: {e}")
        ...
        for ann_data in data.get("annotations", []):
            ...
            if doc_id:
                ...
                try:
                    ...
                    import_errors.append(f"导入注释 {ann.id} 失败: {e}")
        ...
        for bm_data in data.get("bookmarks", []):
            ...
            try:
                ...
                import_errors.append(f"导入书签 {bm.title} 失败: {e}")
        ...
        for plugin_data in data.get("plugins", []):
            ...
            try:
                ...
                import_errors.append(f"导入插件 {plugin.name} 失败: {e}")
        
        # 如果有错误，记录日志（实际应用中应使用 logging 模块）
        if import_errors:
            # 可以在这里添加日志记录
            pass
